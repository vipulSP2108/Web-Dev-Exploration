...
function applyTexture(i, j, data){
    // const basegeometry = new THREE.BoxGeometry(1,0.05,1);
    // const basematerial = new THREE.MeshLambertMaterial({color: data.colour});
    ...
    let boxmaterial;
    // boxmaterial = new THREE.MeshLambertMaterial({color: data.colour});
    if (data.upgrade == 0){
        boxmaterial = new THREE.MeshLambertMaterial({color: data.colour});
    }
    else {
        // const top = new THREE.MeshLambertMaterial({color: data.colour})
        // const side  = new THREE.MeshLambertMaterial({map: textures[randomtexture]});
        boxmaterial = new THREE.MeshLambertMaterial({map: textures[randomtexture]});
        // const top = getTop();
...
const assets = {
    ...
    'road' : (i,j,data) => {
        ...
        // const top = getTop();
        // const side = getSides(road);
        // const boxmaterial = [side, side, top, top, side, side];
        const box = new THREE.Mesh(boxgeometry, boxmaterial);
